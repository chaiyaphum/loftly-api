...
async def _fetch_valuations(
    ...
    the weekly compute job not yet run), we synthesize `FallbackValuation`
    objects from `FALLBACK_VALUATIONS_BY_CODE` (shared with the Selector
    path — see `services/valuation_fallback.py`). This keeps the merchant
    ...
    for row in rows:
        ...
        if code not in out:
            out[code] = valuation
    # Fill in missing codes with fallback values (shared starter table).
    apply_fallback_valuations(out)
    return out
...
def _base_earn_rate_for_category(card: CardModel, category: str | None) -> float:
    ...
    Resolution order (delegated to `valuation_fallback.resolve_earn_rate_key`):
    ...
    key = resolve_earn_rate_key(category, rates)
    if key is None:
        return 0.0
    return rates[key]
...
async def rank_cards_for_merchant(
    ...
    *,
    ...
    for card in cards:
        ...
        elif isinstance(valuation, FallbackValuation):
